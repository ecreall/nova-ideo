...
@interface()
class IFacebookConnector(IConnector):
    pass
...
def context_is_a_facebook(context, request):
    return request.registry.content.istype(context, 'facebook')
...
class AuthSchema(Schema):
    ...
    log_in = colander.SchemaNode(
        ...
        label=_('Login with a Facebook account'),
        description=_('Users can login with their Facebook account.'),
        title='',
        missing=False
    )


class FacebookConnectorSchema(ConnectorSchema):
    """Schema for idea"""

    typ_factory = ObjectData

    name = NameSchemaNode(
        editing=context_is_a_facebook,
        )
...
@implementer(IFacebookConnector)
class FacebookConnector(Connector):
    """FacebookConnector class"""

    type_title = _('Facebook')
    connector_id = FACEBOOK_CONNECTOR_ID
    templates = {'default': 'novaideo:connectors/facebook/views/templates/facebook_bloc.pt',
                 'bloc': 'novaideo:connectors/facebook/views/templates/facebook_bloc.pt',
                 'small': 'novaideo:connectors/facebook/views/templates/facebook_bloc.pt',
                 'popover': 'novaideo:connectors/facebook/views/templates/facebook_bloc.pt'}

    def __init__(self, **kwargs):
        super(FacebookConnector, self).__init__(**kwargs)
    ...
    def get_access_tokens(self, user):
        source_data = user.get_source_data(FACEBOOK_CONNECTOR_ID) if hasattr(user, 'get_source_data') else {}

...
from novaideo.content.processes.novaideo_view_manager.behaviors import  SeeMySupports
...
class SeeMySupportsView(BasicView):
    ...
    def update(self):
        self.execute(None) 
        user = get_current()
        objects = [o for o in getattr(user, 'supports', []) if not('archived' in o.state)]

...
from novaideo.content.processes.novaideo_view_manager.behaviors import  SeeMyIdeas
...
class SeeMyIdeasView(BasicView):
    title = _('My ideas')
    name = 'seemyideas'
    ...
    def update(self):
        self.execute(None)
        user = get_current()
        ideas = getattr(user, 'ideas', [])
        batch = Batch(ideas, self.request, default_size=1)
...
DEFAULTMAPPING_ACTIONS_VIEWS.update({SeeMyIdeas:SeeMyIdeasView})

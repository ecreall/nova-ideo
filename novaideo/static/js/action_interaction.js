function get_action_metadata(action) {
  ...
  var object_views = actions
    .map(function(index) {
      var view = $(
        $(this)
          .parents(
            ".component-obj-view.component-listing-view, .component-obj-view.component-index-view"
          )
          .first()
      )
      return view.attr("id")
    ...
    .get()
  var counters = $(
    '[data-component_type="tab_component"], [data-component_type="navbar_component"], [data-component_type="novaideo_content_nb"]'
  )
    .map(function() {
      return $(this).attr("id")
    })
    .get()
  var contextual_help = $('[data-component_type="contextual-help"]')
    .map(function() {
      return $(this).attr("id")
    })
    .get()
  var steps_navbars = $('[data-component_type="process_steps"]')
    .map(function() {
      return $(this).attr("id")
    })
    .get()
...
function load_views(on_demand, element) {
  ...
  var components = element
    ? element.find('[data-component_type="on-load-view"]')
    : $('[data-component_type="on-load-view"]')
...
function update_modal_action(event) {
  ...
  modal_container.attr(
    "class",
    modal_css_class +
      " action-modal-container action-interation-container modal fade"
  )
...
function _update_sidebar_nav_items(target) {
  ...
  result +=
    '<span class="item-activator desabled glyphicon glyphicon-th"></span>'
  result += "</div>"
...
function open_sidebar_container_item(to_open, interaction_args) {
  ...
  var item = $(
    '.sidebar-nav-items .item[data-target="' + to_open.attr("id") + '"]'
  )
  ...
  function complete() {
    ...
    to_open.fadeIn("fast").promise().done(function() {
...
function update_sidebar_action() {
  ...
  var scroll_bottom = interaction_args
    ? interaction_args.indexOf("scroll-bottom") !== -1
    : false
  var url_attr = { tomerge: "True", coordinates: "main" }
  $.extend(url_attr, get_action_metadata(action))
  loading_progress()
    ...
    include_resources(data["resources"], function() {
      ...
      if (action_body) {
        ...
        if (current_item) {
          ...
          $(sidebar.find(".sidebar-title .entity-title").first()).html(
            new_title
          )
          if (scroll_bottom) {
...
function hide_action_interaction_container(action_container) {
  ...
  if (interaction_type == "modal") {
    action_container.modal("hide")
  }
  if (interaction_type == "inline") {
    action_container.slideDown()
  }
  if (interaction_type == "popover") {
    action_container.css("display", "none")
  }
  if (interaction_type == "slider") {
    _close_slider(action_container)
  }
}
...
$(document).on("submit", "form.novaideo-ajax-form", function(event) {
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data.new_body) {
        $this
          .parents(".views-container")
          .first()
          .html($(jQuery.parseJSON(data.new_body)))
        try {
          deform.processCallbacks()
        } catch (err) {}
        finish_progress()
...
$(document).on("click", function(event) {
  var popover_container = $(
    $(event.target).parents(".action-popover-container")
  )
...
$(document).on("click", ".sidebar-nav-items .item-activator.desabled", function(
  event
) {
  var $this = $(this)
...
$(
  document
).on("click", ".sidebar-nav-items .item-activator:not(.desabled)", function(
  event
) {
  var $this = $(this)

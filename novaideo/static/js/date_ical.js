...
function buildADe(oid) {
  var t = "<div><br/>"
  t +=
    '<div><div class="date-horaire-select" ><label><input type="radio" name="horaire1" id="' +
    oid +
    'selectA" value="à" onclick="affHoraires(\'à\',\'' +
    oid +
    "')\" />&nbsp;&nbsp;à</label></div>"
  ...
  t +=
    '<div><div class="date-horaire-select" ><label><input type="radio" name="horaire1" id="' +
    oid +
    'selectDe" value="de" onclick="affHoraires(\'de\', \'' +
    oid +
    "')\" />&nbsp;de</label></div>"
...
function buildHoraireEtDe(oid) {
  ...
  var t = "<div>"
  t +=
    '&nbsp;&nbsp;&nbsp;<select name="heures3" id="' +
    oid +
    'horaire_heures3" onchange="lhoraireetde(\'' +
    oid +
    "')\">"
  ...
  t += "</select>h "
  t +=
    '<select name="minutes3" id="' +
    oid +
    'horaire_minutes3" onchange="lhoraireetde(\'' +
    oid +
    "')\">"
  t += '<option value="">--</option>'
  var r
  ...
  t += "</select><br />à "
  t +=
    '<select name="heures4" id="' +
    oid +
    'horaire_heures4" onchange="lhoraireetde(\'' +
    oid +
    "')\">"
  t += '<option value="--">--</option>'
  for (var k = 0; k < 24; k++) {
  ...
  t += "</select>h "
  t +=
    '<select name="minutes4" id="' +
    oid +
    'horaire_minutes4" onchange="lhoraireetde(\'' +
    oid +
    "')\">"
  t += '<option value="">--</option>'
...
function buildHoraireDeA(oid) {
  var t = "<div>"
  t +=
    '&nbsp;&nbsp;&nbsp;<select name="heures1" id="' +
    oid +
    'horaire_heures1" onchange="lHoraireDeA(\'' +
    oid +
    "')\">"
  t += '<option value="--">--</option>'
  var i, r
  ...
  t += "</select>h "
  t +=
    '<select name="minutes1" id="' +
    oid +
    'horaire_minutes1" onchange="lHoraireDeA(\'' +
    oid +
    "')\">"
  t += '<option value="">--</option>'
  for (i = 0; i < 60; i = i + 5) {
  ...
  t +=
    '<select name="heures2" id="' +
    oid +
    'horaire_heures2" onchange="lHoraireDeA(\'' +
    oid +
    "')\">"
  t += '<option value="--">--</option>'
  ...
  t += "</select>h "
  t +=
    '<select name="minutes2" id="' +
    oid +
    'horaire_minutes2" onchange="lHoraireDeA(\'' +
    oid +
    "')\">"
  t += '<option value="">--</option>'
  for (i = 0; i < 60; i = i + 5) {
...
function buildEtDe(oid) {
  var t =
    '<div class="date-horaire-select" ><label><input type="radio" name="horaireEtDe" id="' +
    oid +
    'formulaireEtDe" value="etDe" onclick="affHorairesEtDe(\'' +
    oid +
    "')\" />&nbsp;et de</label></div>"
...
function selectMode(mode, element) {
  ...
  _.choIxdate = false
  resetAffichage(oid)
  document.getElementById(oid + "SortirCalendar").innerHTML = buildCal(
    _.curmonth,
    _.curyear,
    oid
  )
  ...
  if (mode == 3) {
    _.choIxDu = false
    var label =
      '<span style="color:#408c40">Choisissez votre début de période</span>'
...
function lhoraireetde(oid) {
  ...
  _.heurEsminutEsEtDe =
    " et de " + heures3 + "h" + minutes3 + " à " + heures4 + "h" + minutes4
  laDate(_.ymdc, oid)
}
...
function affHorairesEtDe(oid) {
  ...
  document.getElementById(oid + "HorairesEtDe").innerHTML = buildHoraireEtDe(
    oid
  )
}
...
function affResultatJusquAu(ymd, oid) {
  ...
  if (_.dE_A) {
    sentence += " " + _.dE_A + " " + _.heurEsminutes + _.heurEsminutEsEtDe
  }
...
function validDuPart(oid /* sav de la première période (Du .. )*/) {
  ...
  var label =
    '<span style="color:#408c40">Choisissez votre fin de période</span>'
  document.getElementById(oid + "messageValidation").innerHTML = label
  document.getElementById(oid + "choixADe").innerHTML = buildADe(oid)
  remove_selected(oid)
}
...
function affBarreDu(ymd, oid /* formate de la zone texte Du...*/) {
  ...
  var button =
    '<button class="btn btn-primary btn-xs" type="button" name="Validez" onclick="validDuPart(\'' +
    oid +
    '\')"><span class="glyphicon glyphicon-play"></span> Valider le début de période</button>'
...
function affBarreAu(ymd, oid /* formate de la zone texte Au... */) {
  ...
  if (date1[_.MONTH] == date2[_.MONTH]) {
    // same month
    p1 = date1[_.DAY]
  } else {
...
function ajoutDatesLes(oid) {
  ...
  t +=
    '<div class="help-user-dates"><span class="glyphicon glyphicon-th-list"></span> Vos dates</div><div name="liste" id="' +
    oid +
    'listeDates" style="width:100%">'
  t += "</div>"
  t +=
    '<p style="text-align:center; margin-top: 15px"><button type="button" class="btn btn-success btn-sm" name="Valider" onclick="generateLesSentence(\'' +
    oid +
    '\')" ><span class="glyphicon glyphicon-ok"></span> Valider ces dates</button></p>'
  t += "</div>"
  return t
}
...
function affBarreLes(ymd, oid /* Affiche de la sélection de date à valider */) {
  ...
  t +=
    '<input class="form-control" type="text" name="showPartSentence" id="' +
    oid +
    'showPartSentence" readonly="true" value="' +
    sentence +
    '">'
  t +=
    '<span class="input-group-btn"><button type="button" name="Valider" class="btn btn-primary" onclick="ajoutTab(\'' +
    oid +
    '\')" ><span class="glyphicon glyphicon-plus"></span>Ajouter à la liste</button></span></div>'
    ...
    if (_.tab[i]) {
      listeDates.append(
        '<div class="date-item">' +
          ...
          i +
          '" class="pull-right btn btn-danger btn-xs remove-date" onclick="Supprimer(\'' +
          oid +
          '\', this);"><span class="glyphicon glyphicon-trash"></span> Supprimer</button></div>'
      )
    }
  }
}
...
function generatePartOfSentence(group, withYear, oid) {
  ...
  if (group.length == 1) {
    ...
    val +=
      " et " +
      group[group.length - 1][_.DAY] +
      " " +
      group[group.length - 1][_.MONTH]
    if (withYear) {
      val += " " + group[0][_.YEAR]
    }
...
function generateLesSentence(oid) {
  ...
  for (i = 1; i < _.tab.length; i++) {
    ...
    if (change_month) {
      if (current_year != lDateTable[_.YEAR]) {
        withYear = true
      }
...
function buildCal(m, y, oid) {
  ...
  var scanfortoday =
    y == todaydate.getFullYear() && m == todaydate.getMonth() + 1
      ? todaydate.getDate()
      : 0

  dim[1] =
    (oD.getFullYear() % 100 !== 0 && oD.getFullYear() % 4 === 0) ||
  ...
  var t = "<table>\n<thead><tr>\n"
  t +=
    '<td class="SortirCalendar_previous_month"><a href="javascript:previousMonth(\'' +
    oid +
    '\')" title="Mois pr&eacute;c&eacute;dent">&lt;&lt;</a></td>\n'
  t +=
    '<td colspan="5" class="SortirCalendar_month_year">' +
    mn[m - 1] +
    " " +
    y +
    "</td>\n"
  t +=
    '<td class="SortirCalendar_next_month"><a href="javascript:nextMonth(\'' +
    oid +
    '\')" title="Mois suivant">&gt;&gt;</a></td>\n'
  t += "</tr>\n<tr>\n"
...
*/
    ...
    if (x == "&nbsp;") {
      ...
      if (_.choixmode === "") {
        // si on a pas encore selectionné de mode, on n'active pas les liens
        t += "<td>" + x + "</td>\n"
      } else {
        t +=
          '<td class="calendar-date" onclick="javascript:laDate(\'' +
          ymd +
          "','" +
          oid +
          "');remove_selected('" +
...
function init_date_ical(date_input) {
  ...
  $(
    $(date_input)
      .parents(".input-group")
      .first()
      .find(".btn.help-activator")
      .first()
  ).on("click", function() {
    var widgt = $(
      $(this)
        .parents(".help-date-wdget")
        .first()
        .find(".helpWriteSeances")
        .first()
    )

...
function get_chrono_expired_msg() {
  return (
    ...
    novaideo_translate("Period expired") +
    "</span>"
  )
}
...
function set_step_time(target) {
  ...
  if (secon.length > 0) {
    chrono_item["secon"] = parseInt(secon.text())
  }
  if (minu.length > 0) {
    chrono_item["minu"] = parseInt(minu.text())
  }
  if (hour.length > 0) {
    chrono_item["hour"] = parseInt(hour.text())
  }
  if (day.length > 0) {
    chrono_item["day"] = parseInt(day.text())
  }
}
...
function chrono(target) {
  ...
  if (chrono_item["secon"] < 0 && total > 0) {
    chrono_item["secon"] = 59
    chrono_item["minu"]--
  }
  total = chrono_item["hour"] + chrono_item["day"]
  if (chrono_item["minu"] < 0 && total > 0) {
    chrono_item["minu"] = 59
    chrono_item["hour"]--
  }
  if (chrono_item["hour"] < 0 && chrono_item["day"] > 0) {
    chrono_item["hour"] = 23
    chrono_item["day"]--
  }
  var secon = $($(target).find("#secon").first())
  ...
  var day = $($(target).find("#day").first())
  if (secon.length > 0) {
    secon.text(chrono_item["secon"])
  }
  if (minu.length > 0) {
    minu.text(chrono_item["minu"])
  }
  if (hour.length > 0) {
    hour.text(chrono_item["hour"])
  }
  if (day.length > 0) {
    day.text(chrono_item["day"])
  }

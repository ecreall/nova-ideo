...
function get_tag(oid, title, body, disabled) {
  return (
    '<div class="panel controllable-item">' +
    '<div class="panel-heading">' +
    '<a  href="#panel-element-' +
    oid +
    '" data-parent="#panel-controllableitems" data-toggle="collapse" class="panel-title">' +
    '<span class="novaideo-icon icon-idea"> </span>  ' +
    title +
    "</a>" +
    '<span class="actions pull-right" data-id="' +
    oid +
    '">' +
    '<span title="Retirer de la liste" class="glyphicon glyphicon-minus-sign ' +
    disabled +
    'del-item"></span></span></div>' +
    '<div class="panel-collapse collapse" id="panel-element-' +
    oid +
    '" style="height: auto;">' +
      ...
      if (data) {
        ...
        if (items.length > 0) {
          related_ideas.append(
            get_tag(data["oid"], data["title"], data["body"], "")
          )
        } else {
          related_ideas.append(
            get_tag(data["oid"], data["title"], data["body"], "disabled-")
          )
        }
        init_delitem()
        if (isnewidea) {
...
$(document).ready(function() {
  ...
  $(".controlled-items").on("delItem", function(event) {
    ...
    for (o in selected_values) {
      if (selected_values[o] == event.itemid) {
        todel = selected_values[o]
      }
    }
    if (todel != -1) {

  ...
  $.each(components_to_update, function(index) {
    ...
    if (original_components.length > 0) {
      ...
      if (data[component_id + ".item_nb"] > 0) {
        new_component =
          '<li data-component_type="navbar_component" class="menu-item counter" id="' +
          component_id +
          '">'
        var ori_view_link = $(original_component.find("a").first())
        if (ori_view_link && ori_view_link.attr("href")) {
          var link_class = ori_view_link.attr("class")
            ? ori_view_link.attr("class")
            : ""
          new_component +=
            '<a class="' +
            link_class +
            '" href="' +
            data[component_id + ".url"] +
            '">'
        } else {
        ...
        new_component +=
          ...
          data[component_id + ".title"] +
          ' <span class=" item-nb">' +
          data[component_id + ".item_nb"]
        if (data[component_id + ".all_item_nb"]) {
          new_component += "/" + data[component_id + ".all_item_nb"]
        }
        new_component += "</span></span>"
        new_component +=
          '<span class="visible-xs-inline-block action-icon ' +
          data[component_id + ".icon"] +
          '"></span>'
        new_component += "</a>"
        new_component += "</li>"
      } else {
        ...
        new_component +=
          '<span class="hidden-xs">' + data[component_id + ".title"] + "</span>"
        new_component +=
          '<span class="visible-xs-inline-block action-icon ' +
          data[component_id + ".icon"] +
          '"></span>'
...
function token_component(data) {
  $.each(data.components, function(index) {
    ...
    $.each(original_components, function(index) {
      ...
      var opposit_class = $this.hasClass("token-success")
        ? ".danger"
        : ".success"
      ...
      if (data.hastoken != undefined) {
        if (data.hastoken) {
          ...
          $(".proposal-support:not(.my-support) .token.active")
            .removeClass("active")
            .addClass("disabled")
        }
      }
...
function novaideo_content_nb_component(data) {
  ...
  $.each(components_to_update, function(index) {
    ...
    if (original_components.length > 0) {
      ...
      if (data[component_id + ".item_nb"] > 0) {
        new_component =
          '<li data-component_type="novaideo_content_nb" class="counter" id="' +
          component_id +
          '"><strong>'
        new_component +=
          data[component_id + ".item_nb"] +
...
function object_view_component(data) {
  ...
  $.each(components_to_update, function(index) {
    ...
    if (original_components.length > 0) {
      if (
        !(
          data.objects_to_hide &&
          $.inArray(component_id, data.objects_to_hide) >= 0
        )
      ) {
        if (data[component_id + ".delated"]) {
          ...
          if (new_body) {
            ...
            var new_view = $("<div>" + data[component_id + ".body"] + "</div>")
              .find("#" + component_id)
              .first()
...
function tab_component_component(data) {
  ...
  $.each(components_to_update, function(index) {
    ...
    if (original_components.length > 0) {
      var original_component = $(original_components[0])
      $(original_component.find(">span").last()).text(
        data[component_id + ".title"]
      )
    }
  })
...
function footer_action_component(data) {
  $.each(data.components, function(index) {
    ...
    $.each(original_components, function(index) {
      var original_component = $(original_components[index])
      var new_component_id = data.has_opposit
        ? "component-footer-action-" + data.new_component_id
        : component_id
      var new_component =
        '<span id="' +
        new_component_id +
...
function dropdown_action_component(data) {
  $.each(data.components, function(index) {
    ...
    $.each(original_components, function(index) {
      var original_component = $(original_components[index])
      var new_component_id = data.has_opposit
        ? "component-dropdown-action-" + data.new_component_id
        : component_id
      var new_component =
        '<span id="' +
        new_component_id +
...
function list_channels_component(data) {
  if (data.removed && data.channel_item) {
    var channel_len = $(
      data.channel_item
        .parents(".channels-block")
        .find(".channel-title .channel-len")
        .first()
    )
  ...
  if (
    !data.removed &&
    data.new_components.length > 0 &&
    data.channels_target.length > 0
  ) {
...
var pseudo_react_components = {
  support_action: [
    nav_bar_component,
    view_title_component,
    removed_items_component,

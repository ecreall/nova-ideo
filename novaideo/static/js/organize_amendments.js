function init_remove_button() {
  ...
  for (i = 0; i < disabledremoveactions.length; i++) {
    ...
    if ($(element).find(".select-item").length == 0) {
      ...
      $(seqremove).attr(
        "onclick",
        "javascript:deform.removeSequenceItem(this);"
      )
    }
  }
}
...
function init_justification() {
  var justification = $(
    $(this)
      .parents(".sequence-item")
      .first()
      .find(".justification-select-item")
      .first()
  )
  if (justification.val() == "") {
    var source = $("#amendment-diff")
    var element = $(
      source.find("span[data-item='" + $(this).data("item") + "']").first()
    )
    var comment = $(
      $(element.find("#explanation_action > button").first()).data("target") +
        " dd#comment"
    )
    if (comment.length > 0) {
...
function init_select() {
  var element = $(this)
    .parents(".drag-drop-panel")
    .first()
    .find(".select-container")
    .last()
  var containers = $(element.parents(".deform-seq-container").first()).find(
    ".select-container"
  )
  var last_element = $(containers[containers.length - 2])
  var las_title = $(
    $(last_element.parents(".sequence-item").first())
      .find(".title-select-item")
      .first()
  ).val()
  var list_split = las_title.split("-")
  var id = parseInt(list_split[list_split.length - 1]) + 1
  var template = $(
    $(element.parents(".deform-seq-container").first())
      .find(".deform-insert-before")
      .first()
  ).attr("item_title")
  $(
    $(element.parents(".sequence-item").first())
      .find(".title-select-item")
      .first()
  ).val(template + id)
  ...
  try {
    init_textarea(
      "#" + justification.attr("id"),
      parseInt(justification.data("limit"))
    )
  } catch (err) {}
}
...
function scrollto() {
  ...
  var scrollvalue =
    ...
    source.height() / 2
  var explanation = $(
    $(element.find("#explanation_action").first())
      .find("dl.explanation-detail")
      .first()
  )
  source.animate(
    {
    ...
    if (!$(".single-amendment-control")[0].checked) {
      if (button.attr("name") != "Cancel") {
        ...
        for (i = 0; i < items.length; i++) {
          ...
          var values = $($(selected).find("option:selected"))
            .map(function() {
              return this.value
            })
            .get()
            .join(", ")
          var justification = $(
            item.find("textarea.justification-select-item").first()
          )
          ...
          if (justification_val == "") {
            ...
            $(commentmessagedanger.find(".errorMsgLbl"))
              .text(
                novaideo_translate("There was a problem with your submission.")
              )
              .show()
      ...
      var justification = $(
        parent.find(".form-group.justification-amendment").first()
      )
...
$(document).ready(function() {
  ...
  $(".single-amendment-control").click(function() {
    ...
    if (this.checked) {
      ...
      form
        .find(".form-group.justification-amendment label")
        .addClass("required")

var open_template =
  '<span class="content-control pull-right glyphicon glyphicon-chevron-up"></span>'
var close_template =
  '<span class="content-control pull-right glyphicon glyphicon-chevron-down"></span>'

function html_content_is_empty(content) {
  return (
    content.val() == '<p><br data-mce-bogus="1"></p>' || content.val() == ""
  )
}
    ...
    if (!event.stop) {
      ...
      $(
        $(this)
          .parents("form")
          .first()
          .find("." + opposit_class + " span.content-control")
          .first()
      ).trigger(click)
    }
  })
...
function init_file_content() {
  ...
  for (var i = 0; i < file_contents.length; i++) {
    ...
    var opposit_is_closed = html_content_is_empty(
      $(
        content
          .parents("form")
          .first()
          .find(".advertising-html-content .html-content-text")
          .first()
      )
    )
    var template = close_template
...
function synchronize_ui_preview() {
  ...
  $(old_preview.find(".preview-advertising-active")).removeClass(
    "preview-advertising-active"
  )
...
$(document).ready(function() {
  ...
  $(".html-content-text").on("ed_init", function(e) {
    if (html_content_is_empty($(this))) {
      var mce_container = $(
        $(this)
          .parents(".advertising-html-content")
          .first()
          .find(".mce-tinymce.mce-container")
          .first()
      )

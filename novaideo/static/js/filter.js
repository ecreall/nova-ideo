...
function init_filter_text_input(filter_container) {
  var filter_input = filter_container
    .find(".filter-input-container input.filter-input")
    .first()
  var form = filter_container.find(".filter-form form")
  var text_to_serach = form
    .find('input.form-control[name="text_to_search"]')
    .first()
...
function filter(event) {
  ...
  if (filter_container_id) {
    target_title = filter_container
      .parents(".content-view")
      .first()
      .find("#" + filter_container_id + "-title")
  }
  if (!target_title || target_title.length == 0) {
    var title_target_id = filter_container
      .parents(".view-filter-container")
      .first()
      .data("title-target")
  ...
  $.post(url, data_get, function(data) {
    ...
    if (data["body"]) {
      ...
      // update tab title
      var source_tab = $(
        '.pontus-main ul.nav.nav-tabs > li > a[href="#' +
          source_tab_id +
          '"] .tab-title'
      )
      ...
      // update view title
      var new_title = filter_container_target
        .find(".filter-activator")
        .first()
        .data("filter_message")
  ...
  $.post(url, data_get, function(data) {
    if (filter_form.length > 0) {
      ...
      if (data["body"]) {
        ...
        // update tab title
        var source_tab = $(
          '.pontus-main ul.nav.nav-tabs > li > a[href="#' + source_tab_id + '"]'
        )
        var tab = result_body
          .find('ul.nav.nav-tabs > li > a[href="#' + source_tab_id + '"]')
          .first()
        var result_target = result_body.find("#" + id)
        var filter_container_target = result_target.find(".filter-container")
        init_filter_text_input(filter_container_target)
        ...
        var new_title = filter_container_target
          ...
          .data("filter_message")
        target_title.html(
          '<div class="panel-title"><h4>' + new_title + "</h4></div>"
        )
        // update results
        target.html(result_target.html())
        try {
...
$(
  document
).on("keypress", ".filter-container .filter-input-container input", function(
  event
) {

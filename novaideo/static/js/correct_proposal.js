function add_edit_correction_form(oid) {
  return (
    '<div data-oid="' +
    oid +
    '" aria-hidden="true" role="dialog" class="edit-item-modal-container modal fade">' +
    ...
    '<span id="' +
    oid +
    '-preload" class="tinymce-preload"></span>' +
    "</div>" +
...
function edit_item(form) {
  ...
  var target = $(
    btn.parents(".correction-container-" + correction_attr).first()
  )
...
function init_correction_navbar() {
  ...
  $(".correction-navbar .correction-nav-actions #hide-all").on(
    "click",
    hide_all
  )
  ...
  if (actions.length == 0) {
    ...
    $(".correction-navbar .correction-nav-actions #accept-all").on(
      "click",
      accept_all
    )
    $(".correction-navbar .correction-nav-actions #refuse-all").on(
      "click",
      refuse_all
    )
  }
...
function init_correction_textarea(oid, lg) {
  ...
  jqoid_preload.click(function() {
    ...
    tinyMCE.init({
      ...
      toolbar:
        "undo redo | fontselect fontsizeselect | forecolor backcolor | bold italic",
      height: 100,
...
$(document).on(
  "click",
  ".correction-action:not(.edit-item-action)",
  correct_handler
)

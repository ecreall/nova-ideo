...
function get_explanation_form() {
  ...
  $.getJSON(url, {}, function(data) {
    ...
    if (action_body) {
      ...
      target
        .find(".explanations-bloc")
        .append(
          '<div class="intention-separator">' +
            novaideo_translate("Or") +
            "</div>"
        )
      try {
...
function submit_explanation() {
  ...
  var relatedexplanation_tag = $(
    form.find("select[name='relatedexplanation']").first()
  )
...
$(document).on("click", ".amendment-toggle", function() {
  ...
  if ($this.hasClass("glyphicon-minus")) {
    ...
    $(
      parent.find(
        "ul.judgment-radio input[type=radio]:not(:checked) ~ .check," +
          "ul.judgment-radio input[type=radio]:not(:checked) ~ label," +
          "ul.judgment-radio input[type=radio]:not(:checked)"
      )
    ).hide()
    $(
      parent.find(

...
;(function() {
  ...
  Utility.getRelativeElementPosition = function(
    source,
    targetElement,
    xDirective,
    yDirective
    ...
    if (isSourceWindow) {
      ...
      switch (yDirective) {
        ...
        case "middle":
          yPosition +=
            source.outerHeight() / 2 - targetElement.outerHeight() / 2
          break
        case "bottom":
  ...
  ConditionQueue.prototype.add = function(
    conditionFunction,
    callback,
    timeout
  ) {
  ...
  Guide.prototype.getPageByName = function(name) {
    var pages = this._pages.filter(function(p) {
  ...
  Guide.prototype.saveStep = function(step) {
    if (this.memoizeLasStep) {
      ...
      Utility.Cookie.set(
        "guideline_" + this.getName() + "_" + Guideline._currentPage,
        stepOffset
      )
    }
  }
  ...
  Guide.prototype.getPageOffset = function() {
    var storedPageOffset = parseInt(
      Utility.Cookie.get("guideline_" + this.getName())
    )
  ...
  Guide.prototype.notifyPageChange = function(pageName) {
    ...
    if (nextPage != null) {
      if (
        nextPage.getName() == pageName &&
        (this.getStepOffset() >= -1 || this.isLastStepOffset())
      ) {
        this.changeToNextPage()
        if (this.memoizeLasStep) {
          var current_step = Utility.Cookie.get(
            "guideline_" + this.getName() + "_" + Guideline._currentPage
          )
    ...
    for (var i = 0; i < this._steps.length; ++i) {
      ...
      if (this._steps[i] != null && pageSteps[i] != null) {
        ...
        if (typeof this._steps[i].continueWhen == "function") {
          Utility.removeHandler(
            this._steps[i]._event_selector_name,
            this._steps[i]._event_event_name,
            this._steps[i]._event_callback_function
          )
          Utility.removeHandler(
            pageSteps[i]._event_selector_name,
            pageSteps[i]._event_event_name,
            pageSteps[i]._event_callback_function
          )
  ...
  var Step = (Guideline.Step = function(options) {
    ...
    if (
      options.continueWhen !== undefined &&
      typeof options.continueWhen != "string"
    ) {
      this.continueWhen = options.continueWhen
    } else {
      if (
        options.continueWhen === undefined &&
        typeof options.showAt == "string"
      ) {
        options.continueWhen = "click " + this.showAt
      }

      this.continueWhen = options.continueWhen
        ? Utility.parseEvent(options.continueWhen)
        : null
    }
    ...
    setTimeout(function() {
      ...
      if (wasHidden) {
        ...
        if (outerScope.continueAfter) {
          outerScope._hideTimeout = setTimeout(function() {
  ...
  Step.prototype.getContentElements = function() {
    ...
    if (this.showSkip) {
      var skipElement = $(
        '<button type="button" class="gl-skip gl-close" title="Close Guide">&times;</button>'
      )
          ...
          if (!error) {
            $(document).ready(function() {
              if (isEventFunction) {
                ...
                outerScope._event_callback_function = function matchCallback() {
                  outerScope.changeToNextStep()
                }
              }
            })
  ...
  Bubble.prototype.positionAt = function(target, align) {
    ...
    try {
      ...
      var position = Utility.getRelativeElementPosition(
        target,
        element,
        alignment.x,
        alignment.y
      ...
      if (this.scrollToItem) {
        if (scroll) {
          if (!$.isWindow(target.get(0))) {
            $("html,body").animate(
              { scrollTop: position.y - element.outerHeight() },
              "slow"
            )
          }
        }
  ...
  Bubble.prototype.getElement = function() {
    if (!this._cachedElement) {
      ...
      var content = this.content
        ? $("<div />").addClass("content ").html(this.content)
        : null

      var cachedElement = $("<div />")
      if (!this.showAtOriginal && this.type == "overlay") {
        content.addClass("col-md-6 col-md-offset-3")
        cachedElement.addClass("gl-overlay row").append(content)
      } else {
        cachedElement
          .addClass("gl-bubble")
          .css({
  ...
  Bubble.prototype.show = function() {
    ...
    if (!this._visible) {
      ...
      if (typeof outerScope.showAt == "function") {
        Guideline.registerConditionCheck(outerScope.showAt, function(
          error,
          result
        ) {
        ...
        Guideline.registerConditionCheck(
          function() {
            return (
              $(outerScope.showAt).length > 0 ||
              outerScope.showAt === "document"
            )
          },
          function(error) {
            container.fadeIn()

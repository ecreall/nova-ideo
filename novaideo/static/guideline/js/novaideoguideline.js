;(function() {
  var NovaIdeoGuideline = (window.NovaIdeoGuideline =
    window.NovaIdeoGuideline || {})

  NovaIdeoGuideline.init = function() {
    var tour_container = $("#novaideo-guide-tour-container")
    NovaIdeoGuideline.logo_url =
      "https://www.nova-ideo.com/static/images/novaideoL.png"
    ...
    if (
      NovaIdeoGuideline.guide_state != "first_start" &&
      NovaIdeoGuideline.guide &&
      !isNaN(NovaIdeoGuideline.guide_value)
    ) {
      $.cookie(
        "guideline_" + NovaIdeoGuideline.guide,
        NovaIdeoGuideline.guide_value
      )
      if (
        NovaIdeoGuideline.page &&
        !isNaN(NovaIdeoGuideline.page_value) &&
        NovaIdeoGuideline.page_value > 0
      ) {
        $.cookie(
          "guideline_" + NovaIdeoGuideline.guide + "_" + NovaIdeoGuideline.page,
          NovaIdeoGuideline.page_value
        )
      }
    }
  }
  ...
  NovaIdeoGuideline.onStepShown = function(step) {
    if (NovaIdeoGuideline.update_url) {
      ...
      $.post(NovaIdeoGuideline.update_url, {
        guide,
        guide_value,
        page,
        page_value,
    ...
    if (current_page) {
      ...
      if (current_page_str == undefined) {
        ...
        if (isNaN(current_step) || current_step <= steps.length) {
          ...
          var showAt = isNaN(current_step)
            ? $(".contextual-help-toggle")
            : $(steps[current_step].showAt)
  ...
  NovaIdeoGuideline.next_btn =
    "<button type='button' class='btn btn-xs btn-primary'>" +
    "<span class='glyphicon glyphicon-forward' aria-hidden='true'></span> " +
    NovaIdeoGuideline._("Next") +
    "</button>"

  NovaIdeoGuideline.prev_btn =
    "<button type='button' class='btn btn-xs btn-primary'>" +
    "<span class='glyphicon glyphicon-backward' aria-hidden='true'></span> " +
    NovaIdeoGuideline._("Previous") +
    "</button>"

  NovaIdeoGuideline.end_btn =
    "<button type='button' class='btn btn-xs btn-success'>" +
    "<span class='glyphicon glyphicon-ok' aria-hidden='true'></span> " +
    NovaIdeoGuideline._("End") +
    "</button>"

  NovaIdeoGuideline.stepControlContainer =
    "<div class='btn-group btn-group-xs step-control-container'/>"
})()
